...
app.post('/express/register', async (req, res) => {
  ...
  if (name && email && password) {
    try {
      ...
      const data = await flaskApiResponse.json();
      console.log(`Response from Flask API for registration: ${JSON.stringify(data)}`);
      
      // Check the response status and send an appropriate response to the client
      if (data.status === 'success') {
        res.status(200).json({ status: 'success', message: 'User registered successfully', user: data.user });
      } else {
